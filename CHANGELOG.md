...
- Добавлен `run-perplexity.ps1`: он получает путь к `request.json`, проверяет
  модель и файлы, безопасно собирает вопрос с разрешёнными материалами и затем
  вызывает неизменяемый транспортный скрипт.
- Добавлены шаблоны `request.example.json` и `question.example.md`.
- Установка поддерживает Claude Code, Codex и Cursor; все получают одинаковый
  комплект файлов навыка.
- Правило Cursor больше не содержит встроенную копию транспортного кода.
- Исправлен шаблон манифеста: он ссылается на существующий файл вопроса.
- Обновлены таблицы моделей и стоимости Sonar API по официальной документации;
  добавлены дата проверки и прямые ссылки на карточки моделей.
