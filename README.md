...
- Провести ревью плана или архитектурного решения с внешней точки зрения
...
```
просто "спроси у перплексити как решить проблему"
...
Агент (Claude Code и Cursor) выбирает модель автоматически по контексту задачи:

| Задача | Модель |
|--------|--------|
| Быстрый факт, уточнение API, короткий вопрос | `sonar` |
| Ревью плана, архитектурное решение, глубокое исследование | `sonar-pro` |
| Многошаговое рассуждение, сложный анализ | `sonar-reasoning-pro` |
| Синтез из множества источников, исчерпывающее исследование | `sonar-deep-research` |

Список моделей может измениться. Актуальный перечень: [docs.perplexity.ai](https://docs.perplexity.ai/).

### Результаты тестирования моделей (март 2026)

Вопрос: «Какая последняя версия .NET SDK вышла в 2025 году?»

| Модель | Характер ответа |
|--------|----------------|
| `sonar` | Кратко и по делу: .NET 10.0.1 (декабрь 2025) |
| `sonar-pro` | Подробнее, с контекстом по версиям SDK |
| `sonar-reasoning-pro` | Чётко и структурированно, с временным контекстом |
| `sonar-deep-research` | Полный аналитический отчёт (>30 КБ) — избыточен для фактических вопросов |

 генерирует очень большой объём текста — подходит только для задач, где нужен исчерпывающий документ.
