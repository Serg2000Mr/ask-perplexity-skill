...
| Model ID | Что делает | Когда полезна |
|----------|------------|---------------|
| `sonar` | Быстрый веб-поиск, Q&A, суммаризация | Короткие фактические вопросы, уточнения API |
| `sonar-pro` | Глубокий поиск с расширенным контекстом (200k) | Анализ, ревью, исследование |
| `sonar-reasoning-pro` | Многошаговое рассуждение, chain-of-thought | Сложный анализ, структурированные выводы |
| `sonar-deep-research` | Синтез из сотен источников | Исчерпывающий документ по теме |
...
**Вывод из тестирования** (вопрос: «Какая последняя версия .NET SDK вышла в 2025 году?»):

- `sonar` — кратко и по делу: .NET 10.0.1 (декабрь 2025)
- `sonar-pro` — лучший баланс для ревью и анализа
- `sonar-reasoning-pro` — когда важна структура рассуждения
- `sonar-deep-research` — когда нужен большой итоговый документ (>30 КБ), избыточен для фактических вопросов
