...
## Модели Perplexity (поисковые)

| Модель | Описание |
|--------|----------|
| `sonar` | Быстрый веб-поиск на базе Llama (используется по умолчанию) |
| `sonar-pro` | Глубокий поиск с расширенным рассуждением (дороже) |

Perplexity также предоставляет доступ к моделям OpenAI, Anthropic, Google и другим — см. [документацию API](https://docs.perplexity.ai/).
